...
{
    public partial class EditFuncionarioComponent : ComponentBase
    {
        ...
        [Parameter]
        public dynamic FuncionarioID { get; set; }
        ...
        {
            var bibliotecaDbGetFuncionarioByFuncionarioIdResult = await BibliotecaDb.GetFuncionarioByFuncionarioId(FuncionarioID);
            funcionario = bibliotecaDbGetFuncionarioByFuncionarioIdResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(A2019128143.Models.BibliotecaDb.Funcionario args)
        {
            try
            {
                var bibliotecaDbUpdateFuncionarioResult = await BibliotecaDb.UpdateFuncionario(FuncionarioID, funcionario);
                DialogService.Close(funcionario);
            }
            catch (System.Exception bibliotecaDbUpdateFuncionarioException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update Funcionario" });
            }
        }
