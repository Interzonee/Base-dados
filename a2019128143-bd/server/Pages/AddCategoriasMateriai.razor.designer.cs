...
{
    public partial class AddCategoriasMateriaiComponent : ComponentBase
    {
        ...
        {
            categoriasmateriai = new A2019128143.Models.BibliotecaDb.CategoriasMateriai(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(A2019128143.Models.BibliotecaDb.CategoriasMateriai args)
        {
            try
            {
                var bibliotecaDbCreateCategoriasMateriaiResult = await BibliotecaDb.CreateCategoriasMateriai(categoriasmateriai);
                DialogService.Close(categoriasmateriai);
            }
            catch (System.Exception bibliotecaDbCreateCategoriasMateriaiException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new CategoriasMateriai!" });
            }
        }
