...
{
    public partial class EditAvaliacoComponent : ComponentBase
    {
        ...
        [Parameter]
        public dynamic AvaliacaoID { get; set; }
        ...
        {
            var bibliotecaDbGetAvaliacoByAvaliacaoIdResult = await BibliotecaDb.GetAvaliacoByAvaliacaoId(AvaliacaoID);
            avaliaco = bibliotecaDbGetAvaliacoByAvaliacaoIdResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(A2019128143.Models.BibliotecaDb.Avaliaco args)
        {
            try
            {
                var bibliotecaDbUpdateAvaliacoResult = await BibliotecaDb.UpdateAvaliaco(AvaliacaoID, avaliaco);
                DialogService.Close(avaliaco);
            }
            catch (System.Exception bibliotecaDbUpdateAvaliacoException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update Avaliaco" });
            }
        }
